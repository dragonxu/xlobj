...
//任务优先级
#define ETHI_TASK_PRIO		18
//任务堆栈大小	
#define ETHI_STK_SIZE 		350  
//任务句柄
TaskHandle_t ETHITask_Handler;
//任务函数
...
#endif
	vSemaphoreCreateBinary(s_xSemaphore); //创建二值量
	//数据接收创建任务
  xTaskCreate((TaskFunction_t )ethernetif_input, 
							(const char*    )"EthIf", 
							(uint16_t       )	ETHI_STK_SIZE, 
							(void*          ) netif,
							(UBaseType_t    ) ETHI_TASK_PRIO,
							(TaskHandle_t*  ) ETHITask_Handler);	
	ETH_Start(); //开启MAC和DMA				
	return ERR_OK;
} 
